...
public class DAOOrder {
	...
	private static final String INSERT = "INSERT INTO `order` (id_pizza, id_client, quantity, status, "
			+ "amount) VALUES (?,?,?,?,?)";
	private static final String SELECT_DETAILS_ORDER = "SELECT o.quantity, p.name AS pizza, "
			+ "e.name AS employee, o.`status` FROM `order` o INNER JOIN pizza p ON p.id = o.id_pizza "
			+ "INNER JOIN employee e ON e.id = o.id_employee";
